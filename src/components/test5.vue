<template>
  <div>
    ...
    <divider>default(分割线)</divider>
    <box gap="10px 10px">
      <x-button>submit(默认)</x-button>
      <x-button type="primary">primary(绿色)</x-button>
      <x-button type="warn">Delete(红色)</x-button>

      <divider>action type</divider>
      <x-button type="primary" action-type="button">submit(绿色)</x-button>
      <x-button type="warn" action-type="reset">reset(红色)</x-button>
      ...
      <divider>combined with flexbox(一行展示多个按钮)</divider>
      <flexbox>
        <flexbox-item>
...
<script>
import { XButton, Box, GroupTitle, Group, Flexbox, FlexboxItem, Divider, XHeader } from 'vux'
